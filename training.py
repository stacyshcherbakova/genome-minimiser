...
def train_cyclic_KL_annealing_additional_loss_SCALED(model, optimizer, scheduler, n_epochs, train_loader, val_loader, min_beta, max_beta, gamma_gene_abundance_start, gamma_gene_abundance_end, gamma_genome_size_start, gamma_genome_size_end, max_norm, lambda_l1):
    ...
    for epoch in range(n_epochs):
        gamma_gene_abundance = gamma_gene_abundance_start + (gamma_gene_abundance_end - gamma_gene_abundance_start) * epoch / n_epochs
        gamma_genome_size = gamma_genome_size_start + (gamma_genome_size_end - gamma_genome_size_start) * epoch / n_epochs
        model.train()
        ...
        for batch in train_loader:
            ...
            loss = reconstruction_loss + (beta * kl_divergence_loss) + (gamma_genome_size * genome_size_loss) + l1_penalty + (gamma_gene_abundance * gene_abundance_loss)
            ...
            scaled_gene_abundance_loss = (gamma_gene_abundance * gene_abundance_loss) / loss
            scaled_genome_size_loss = (gamma_genome_size * genome_size_loss) / loss
            ...
            epoch_train_gene_abundance_loss += scaled_gene_abundance_loss.item()
            epoch_train_genome_size_loss += scaled_genome_size_loss.item()
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                loss = reconstruction_loss + (beta * kl_divergence_loss) + (gamma_genome_size * genome_size_loss) + (gamma_gene_abundance * gene_abundance_loss)
                ...
                epoch_val_gene_abundance_loss += scaled_gene_abundance_loss.item()
                epoch_val_genome_size_loss += scaled_genome_size_loss.item()
    ...
    print("Train Gene Abundance Loss:", train_gene_abundance_loss_vals[:10])
    print("Train Genome Size Loss:", train_genome_size_loss_vals[:10])
    ...
    plt.tight_layout()
    plt.savefig(folder + "train_losses_separated.pdf", format="pdf", bbox_inches="tight")
    plt.show()
    ...
    plt.legend()
    plt.savefig(folder+"train_losses_log_scale.pdf", format="pdf", bbox_inches="tight")
    plt.show()
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(folder+"train_losses_1.pdf", format="pdf", bbox_inches="tight")
    plt.show()
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(folder+"validation_losses_1.pdf", format="pdf", bbox_inches="tight")
    plt.show()
...
def train_cyclic_KL_annealing_additional_loss_2(model, optimizer, scheduler, n_epochs, train_loader, val_loader, min_beta, max_beta, gamma_start, gamma_end, max_norm, lambda_l1):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(folder+"train_losses_2.pdf", format="pdf", bbox_inches="tight")
    plt.show()
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(folder+"validation_losses_2.pdf", format="pdf", bbox_inches="tight")
    plt.show()
